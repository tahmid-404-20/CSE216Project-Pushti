...
router.route("/").get((req, res, next) => {
  if (req.session.user) {
    var title =
      process.env.APPNAME.toString() + " - " + "Farmer - Send Feedback";
    res.render("sendFeedbackFarmer", {
      title: title,
    });
  } else {
      ...
      oracleDB.getConnection(
        ...
        (err, con) => {
          if (err) {
            ...
            const p = `SELECT count(*) from FarmerFeedback`;
            con.execute(p, [], (e, r) => {
              if (e) {
                res.send({
                  error: e,
                ...
                const q = `INSERT INTO FarmerFeedback VALUES(${feedback_id},'${details}',SYSDATE,'F',${farmer_id})`;
